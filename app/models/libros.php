...
class Libros extends BaseDeDatos {
    ...
    public function getAll() {
        return $this->executeQuery("Select id_libro, titulo, descripcion, categoria, autor, date_format(fecha_publicacion,'%d-%m-%Y') as fecha_pub
         from categorias inner join (autores inner join libros using(id_autor)) using(id_categoria) order by titulo");
    }
    ...
    public function save($data,$imgp,$imgm,$imgg) {
        return $this->executeInsert("insert into libros set titulo='{$data["titulo"]}', descripcion='{$data["descripcion"]}', 
        id_autor='{$data["id_autor"]}', id_categoria='{$data["id_cate"]}', fecha_publicacion='{$data["fecha_publicacion"]}', 
        fotop='{$imgp}',fotom='{$imgm}',fotog='{$imgg}'");
    }
    ...
    public function getLibrosReporte($data) {
        ...
        if ($data["idcate"]!="0") {
            $condicion.=" and b.id_categoria='{$data["idcate"]}'";
        }
        return $this->executeQuery("Select a.*, b.categoria, c.autor, date_format(a.fecha_publicacion,'%d-%m-%Y') as fecha_pub 
        from autores c inner join (categorias b inner join libros a using(id_categoria)) using(id_autor)
        where 1=1 $condicion");
    }
}
